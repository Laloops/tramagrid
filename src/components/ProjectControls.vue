<script setup>
    ...
    // Adicione getParams e eventBus na importação
    import { uploadImage, generateGrid, updateParams, sessionId, getParams, eventBus } from '../api.js'
    ...
    // Função para sincronizar o Frontend com o Backend
    ...
    // Escuta o evento 'refresh' (disparado pelo Undo)
    onMounted(() => {
    ...
    async function handleUpload(e) {
      ...
      try { await uploadImage(file); alert('Imagem carregada!'); } 
      ...
      <div class="project-controls">
        ...
        <div class="control-group">
            ...
            <div class="gauge-row">
                <label>
                    <span>Pontos:</span>
                    <input v-model.number="gaugeStitches" type="number" class="input-number" />
                </label>
                <label>
                    <span>Carr.:</span>
                    <input v-model.number="gaugeRows" type="number" class="input-number" />
                </label>
            </div>
            <small class="hint">Define a proporção (retangular ou quadrada) do ponto.</small>
        </div>
    
        <div class="separator"></div>
    
        <div class="control-group">
          <label title="Reduz os níveis de cor para criar um efeito 'chapado'.">
            <span>Posterizar: {{ posterize }}</span>
            <input v-model.number="posterize" @change="generate" type="range" min="1" max="8" step="1" class="slider" />
          </label>
          
          <label>
            <span>Sombras (Gama): {{ gamma }}</span>
        ...
        <button @click="generate" class="btn success full-width mt-2">▶ Gerar / Atualizar</button>
      </div>
    </template>
    ...
    .btn-toggle {
        background: #333; border: 1px solid #555; color: #aaa; padding: 4px 12px; border-radius: 15px; cursor: pointer; transition: 0.3s; font-size: 0.8rem; font-weight: bold;
    }
