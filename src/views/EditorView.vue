<script setup>
    ...
    async function handleFinishProject() {
      ...
      if (!user) {
          if(confirm("Você precisa estar logado para salvar.\nIr para login?")) router.push('/login')
          return
      }
  
      const { data: profile } = await supabase.from('profiles').select('*').eq('id', user.id).single()
      if (!profile) return alert("Erro: Perfil não encontrado.")
      ...
      try {
          ...
          // Correção: Usa /api/grid/ em vez de /api/original/
          ...
          console.log("✅ Grade recebida. Iniciando upload...");
  
          // B. Upload
          const imageBlob = base64ToBlob(imgData.image_base64)
          const fileName = `${user.id}/${Date.now()}_grid.png` // Nome sugere que é o grid
          ...
          const { error: projectError } = await supabase
              ...
              .insert({
                  user_id: user.id,
                  name: nomeProjeto,
                  image_url: publicUrl, // URL da grade bonita
                  settings: currentParams // Configurações para reedição futura
              })
