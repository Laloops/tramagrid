<script setup>
  ...
  async function handleLogin() {
    console.log("🔵 [LoginView] Iniciando login com email:", email.value)
    loading.value = true
    message.value = ''
    ...
    if (error) {
        console.error("🔴 [LoginView] Erro no signInWithOtp:", error)
        message.value = 'Erro: ' + error.message
    } else {
        console.log("🟢 [LoginView] Link mágico enviado com sucesso!", data)
        message.value = '✨ Link enviado para seu e-mail!'
    }
    loading.value = false
  }
  
  async function handleSocialLogin(provider) {
    console.log(`🔵 [LoginView] Iniciando login social com ${provider}`)
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: provider,
      options: { emailRedirectTo: window.location.origin }    })
    
    if (error) {
        console.error("🔴 [LoginView] Erro no signInWithOAuth:", error)
        alert("Erro no login social: " + error.message)
    } else {
        console.log("🟢 [LoginView] Redirecionando para OAuth...", data)
    }
  }
</script>
...
<style scoped>
/* (O CSS permanece o mesmo do original) */
